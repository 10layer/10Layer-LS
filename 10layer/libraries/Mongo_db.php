 ...
 * @author		Jason Norwood-Young | www.10layer.com | jason@10layer.com
...
class Mongo_db {
	...
	*/
	...
	public function get($collection = "") {
		...
		foreach($documents as $doc):
			...
			$returns[] = (object) $doc; //Changed by Jason Norwood-Young 
	...
	public function get_light($urlid){
		...
		$item = $this->db->content->findOne(array("_id" => $urlid)); //$this->command($query); // $this->db->content->find(array("_id" => $urlid));
		return $item;
	}
	...
	*	--------------------------------------------------------------------------------
	*
	*	Increment value atomically JNY
	*
	*	@usage : $this->mongo_db->increment('foo', $data = array('qty'=>1));
	*/
	
	 public function increment($collection = "", $data) {
		if(count($data) == 0 || !is_array($data))
			show_error("Nothing to update in Mongo collection or update is not an array", 500);
		...
		try {
			...
			show_error("Increment of data into MongoDB failed: {$e->getMessage()}", 500);
		}		
	}
	...
	*/
	...
	private function connect() {
		...
		try {
			...
			if ($this->CI->uri->segment(2)!="healthchecks") {
	...
	*/
	...
	private function connection_string() {
		...
		$this->user = trim($this->CI->config->item('mongo_username'));
		$this->pass = trim($this->CI->config->item('mongo_password'));
